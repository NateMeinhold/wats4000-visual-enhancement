<template>
  <div>
    ...
    <div class="word-list-container">
      <h2>Word List</h2>
      <ul class="word-list">
        <!-- Transition-group around the list item here to animate items in the word list. -->
    ...
    <div class="results-container">
      <!-- Spinner here to show when search is in progress. -->
      <spinner v-if="showSpinner"></spinner>
      <h2 v-if="results && results.length > 0">{{ results.length }} Words Found</h2>
      <ul v-if="results && results.length > 0" class="results">
        <!-- Transition-group around the list item here to animate items in the results list. -->
...
// Note: vue2-animate added using the require statement because it is a CSS file
  ...
  methods: {
    addWord: function(word) {
      if (this.wordList.indexOf(word) === -1) {
        ...
        // Message to this.messages to reflect word added for user.
        this.messages.push({
          type: "success",
        ...
        // Message to this.messages to reflect word in wordlist for user. 
        this.messages.push({
          type: "info",
    ...
    removeWord: function(word) {
      ...
      // Message to this.messages to reflect word removed for user.
      this.messages.push({
        type: "success",
    ...
    findWords: function() {
      // Showing spinner when API request begins.
      ...
      axios
        ...
        .catch(error => {
          // Turn off spinner
          this.showSpinner = false;
          // Add message to this.messages to display the errors.
          this.showSpinner = false;
          this.messages.push({
            type: "error",
            text: error.message
